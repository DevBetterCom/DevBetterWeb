...
public class MemberSetup
{
  ...
  public async Task SetsUpNewMember()
  {
    ...
    _invitationRepository.FirstOrDefaultAsync(Arg.Any<InvitationByInviteCodeSpec>(), CancellationToken.None)!.Returns(Task.FromResult(_invitation));
    _invitationRepository.ListAsync(Arg.Any<ActiveInvitationByEmailSpec>(), CancellationToken.None).Returns(Task.FromResult(new List<Invitation>()) );
