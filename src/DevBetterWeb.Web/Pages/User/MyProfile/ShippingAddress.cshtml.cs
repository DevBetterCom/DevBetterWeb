...
namespace DevBetterWeb.Web.Pages.User
{
	...
	public class ShippingAddressModel : PageModel
	{
		...
		{
			var (member, userId) = await GetMemberAsync(User);
			if (member is null && userId is null) return;
			if (member == null)
			{
		...
		private async Task<(Member? member, string? userId)> GetMemberAsync(ClaimsPrincipal user)
		{
